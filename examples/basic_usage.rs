...
use nexa_net::{
    api::sdk::{NexaClient, NexaClientBuilder, CapabilityBuilder, DiscoveryFilters},
    discovery::capability::{CapabilityRegistry, QualityMetrics},
    economy::channel::ChannelManager,
    identity::{Did, KeyPair, IdentityKeys},
    types::{CapabilitySchema, ServiceMetadata, EndpointDefinition},
};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("   Public key: {} bytes", identity.signing_key.public_key().to_bytes().len());
    ...
    let capability = CapabilityBuilder::new(&did, "Translation Service")
        ...
        .endpoint(EndpointDefinition {
            ...
            cost_per_call: 100,  // 100 micro-tokens per call
            avg_latency_ms: 50,
        })
        .build();
    ...
    println!("   Found {} capabilities matching 'translation'", results.len());
    
    // Update quality metrics
    registry.update_quality(did.as_str(), QualityMetrics {
        success_rate: 0.98,
        avg_latency_ms: 45.0,
        total_calls: 1000,
    ...
    println!("   Found {} high-quality capabilities (>95%% success)", high_quality.len());
    ...
    println!("   Total capacity: {}", channel.balance_a + channel.balance_b);
    ...
    println!("   Registry: {} capabilities, {} unique tags", 
        registry_stats.total_capabilities,
        registry_stats.unique_tags
    );
    
    let channel_stats = channel_manager.stats();
    println!("   Channels: {} open, {} total capacity", 
        channel_stats.open_channels,
        channel_stats.total_capacity
    );
