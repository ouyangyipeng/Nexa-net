...
use nexa_net::types::{Did, CapabilitySchema, ServiceMetadata, EndpointDefinition, RouteContext};
...
fn test_payment_channel_flow() {
    ...
    let channel = channel_manager.open(payer.clone(), payee.clone(), 10000, 10).unwrap();
    ...
    channel_manager.update_balances("channel-1", 9950, 60).unwrap();
...
fn test_full_workflow() {
    ...
    let cost = 50u64;
    channel_manager.update_balances("channel-1", 1000 - cost, 10 + cost).unwrap();
