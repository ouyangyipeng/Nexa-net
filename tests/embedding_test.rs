...
use nexa_net::discovery::embedding::utils::{cosine_similarity, euclidean_distance, normalize};
use nexa_net::discovery::{Embedder, EmbeddingConfig, MockEmbedder, Vectorizer, VectorizerBuilder};
use std::sync::Arc;
...
fn test_vectorizer_builder() {
    let vectorizer = VectorizerBuilder::new().mock(512).build().unwrap();
...
fn test_api_embedder_not_implemented() {
    ...
    assert!(
        result.is_err(),
        "API embedder should not be implemented yet"
    );
}
...
mod onnx_tests {
    ...
    fn test_onnx_semantic_similarity() {
        ...
        if model_path.exists() {
            ...
            assert!(
                sim_12 > 0.7,
                "Very similar intents should have high similarity"
            );
        }
    }
}
